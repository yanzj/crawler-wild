...
def goods_images(goods_url):
    ...
    # 计算晒图页数,向上取整
    pages = int(math.ceil(imageListCount / 10))
    for page in range(1, pages + 1):
        ...
        club_req = requests.get(url=club_url, params=club_params, headers=club_headers, verify=False, allow_redirects=False)
        # club_req = requests.get('https://club.jd.com/discussion/getProductPageImageCommentList.action?_=153338172107&isShadowSku=0&page=1&pageSize=10&productId=13970262977')
        print(club_req.text)
...
def download_image(path, image_url):
    ...
    with closing(requests.get(image_url, headers=download_headers, stream=True)) as response:
        ...
        if response.status_code == 200:
            sys.stdout.write(filename + u'下载中:\n')
            sys.stdout.write(u'    [文件大小]:%0.2f MB\n' % (content_size / chunk_size / 1024))
            ...
            with open(image_path, 'wb') as file:
                for data in response.iter_content(chunk_size=chunk_size):
                    ...
                    size += len(data)
                    file.flush()
                    sys.stdout.write(u'    [下载进度]:%.2f%%' % float(size / content_size * 100) + '\r')
                    sys.stdout.flush()
...
def run(path, keyword, num):
    ...
    store_dir = os.path.join(path, keyword)
    print(store_dir)
    ...
    for goods_url in goods_urls[:num]:
        ...
        for image_url in image_urls:
            download_image(store_dir, image_url)
